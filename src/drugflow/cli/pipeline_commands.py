...
def quick_profile(ctx, input_path, output_dir, smiles_col, id_col):
    ...
    from drugflow.phase1.analysis.similarity import diversity_analysis
    from drugflow.phase1.visualization.structure import draw_molecule_grid
    from drugflow.phase1.visualization.distributions import (
        plot_property_distributions,
        plot_property_scatter,
    )
    from drugflow.phase1.visualization.chemical_space import plot_chemical_space_pca

    Path(output_dir).mkdir(parents=True, exist_ok=True)

    # Step 1: Load & Validate
    click.echo("=" * 60)
    click.echo("DrugFlow Quick Profile")
    click.echo("=" * 60)
...
def run(ctx, config):
    ...
    from drugflow.core.config import PipelineConfig
    from drugflow.phase1.data.loaders import load_file
    from drugflow.phase1.data.validators import validate_dataset
    from drugflow.phase1.data.standardizer import standardize_dataset
    from drugflow.phase1.data.writers import write_file
...
def screen_and_score(ctx, input_path, reference, model_path, output_dir,
    ...
    click.echo("=" * 60)
    click.echo("DrugFlow Screen & Score Pipeline")
    click.echo("=" * 60)
    ...
    if reference:
        ...
        from drugflow.phase2.screening.similarity_screen import screen_similarity
