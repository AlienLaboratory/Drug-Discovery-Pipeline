...
def descriptors(ctx, input_path, output, compute_all, names, list_available):
    ...
    from claudedd.phase1.analysis.descriptors import (
...
def filter_cmd(ctx, input_path, output, rejected, lipinski, veber,
    ...
    if rejected:
        from claudedd.core.models import MoleculeDataset, MoleculeStatus
...
def similarity(ctx, input_path, query_smiles, fp_type, metric, top_k, output):
    ...
    from claudedd.phase1.data.loaders import load_file
    from claudedd.phase1.data.validators import validate_dataset
    from claudedd.phase1.analysis.fingerprints import compute_fingerprints_dataset, compute_morgan, compute_maccs
    from claudedd.phase1.analysis.similarity import find_nearest_neighbors, diversity_analysis
    from claudedd.phase1.data.writers import write_file
...
def score(ctx, input_path, output, rank, top_n, activity_weight, dl_weight, sa_weight):
    ...
    from claudedd.phase2.scoring.ranking import rank_molecules, export_ranked_results
    from claudedd.phase1.data.writers import write_file
